...
plot_labels     = ["surface"] + modes
path_to_bare="{}-CO/top*".format(surface)
path_to_ads_list=["{}-CO-CO/{}*".format(surface,subfix) 
                  for subfix in modes]

params_json="parameters.json"
with open(params_json) as fj:
    parameters=json.load(fj)

# ------------------------------------ def functions
U_SHE           = -5.38
A_ads           = -43.70395510*ry2ev

switch_on_points_label=False
...
ne_slab=parameters[surface]["bare"]["nelect-bare"]
...
colors_list=["red","blue","green","orange","orange","dimgray","purple","dodgerblue","salmon","firebrick"]
...
for counter, path_to_ads in enumerate(path_to_ads_list):
    ...
    xmin,xmax=x_slabads.min(),x_slabads.max()
    func_slabads=fitfunction(x_slabads,y_slabads)
    xt_slabads=np.linspace(xmin,xmax,1000)
