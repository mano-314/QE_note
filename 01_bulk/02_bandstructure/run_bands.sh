...
# 2) start from charge density from previous scf calculation and expand on particular k-path
mpirun -hostfile $PBS_NODEFILE -n $NPROCS pw.x -in nscf.in > nscf.out

# 3) extract eigenvalues and output in ready-to-plot format
mpirun -hostfile $PBS_NODEFILE -n $NPROCS bands.x -in bands.in > bands.out
